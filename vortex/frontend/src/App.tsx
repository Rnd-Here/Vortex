...
function ChatPlatform() {
  ...
  const handleInitialSetup = async () => {
    if (!MOCK_CONFIG.enabled && !newApiKey.startsWith('sk-')) {
  ...
  const handleUpdateApiKey = () => {
    if (confirmText.toUpperCase() !== 'YES') {
      alert('Please type YES to confirm changes.');
      return;
    }
    ...
    <div className="flex h-screen bg-[#0a0a0f] text-[#e2e2e2] font-sans selection:bg-[#00f2ff]/30">
      ...
      <aside className="w-72 bg-[#0d0d14] border-r border-[#1f1f2e] flex flex-col hidden lg:flex">
        ...
        <div className="flex-1 overflow-y-auto px-4 py-2 space-y-2 custom-scrollbar">
          <div className="text-[10px] font-bold text-[#4a4a6a] mb-4 px-3 uppercase tracking-[0.2em]">Live Connections</div>
          {sessions.map(s => (
          ...
          <div className="flex items-center gap-4">
            {activeSession ? (
              <div className="flex flex-col">
                <span className="text-sm font-bold text-[#00f2ff] tracking-wide uppercase">{activeSession.name || `${activeSession.mode} PLAYGROUND`}</span>
        ...
        <div className="flex-1 overflow-y-auto p-6 lg:p-10 space-y-8 custom-scrollbar">
          {!activeSession && (
            <div className="h-full flex flex-col items-center justify-center text-center max-w-2xl mx-auto">
              ...
              <h2 className="text-3xl font-bold text-white mb-4 tracking-tight">Vortex Playground Terminal</h2>
              <p className="text-[#8e8eaf] text-lg leading-relaxed mb-8">{isSetupComplete ? 'Initialize a new playground or select a connection from history.' : 'Please configure your LiteLLM API Key to establish a secure connection.'}</p>
              <button onClick={() => protectedAction(handleOpenNewChat)} className="bg-[#00f2ff] hover:bg-[#00d8e6] text-[#0a0a0f] px-8 py-3 rounded-xl font-bold transition-all">{isSetupComplete ? 'Launch New Playground' : 'Configure Endpoint'}</button>
            </div>
          )}
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-6 max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-2 duration-300 ${msg.role === 'user' ? 'justify-end' : ''}`}>
              {msg.role !== 'user' && ( <div className="w-10 h-10 rounded-xl bg-[#1a1a2e] border border-[#00f2ff]/30 flex items-center justify-center flex-shrink-0 shadow-[0_0_15px_rgba(0,242,255,0.1)]"><Bot size={22} className="text-[#00f2ff]" /></div> )}
              <div className={`px-6 py-4 rounded-2xl max-w-[85%] text-[15px] leading-relaxed border ${msg.role === 'user' ? 'bg-gradient-to-br from-[#00f2ff] to-[#7000ff] text-[#0a0a0f] font-medium border-transparent rounded-tr-none' : 'bg-[#12121e] text-[#e2e2e2] border-[#1f1f2e] rounded-tl-none shadow-xl'}`}>
      ...
      {/* Modals: Settings, New Session, Rename, Delete */}
      {isSettingsModalOpen && (
            ...
            <div className="space-y-6 relative">
              {availableModels.length > 0 ? (
                <div className="space-y-6 animate-in fade-in slide-in-from-top-2">
                  ...
                  <button onClick={handleStartInteraction} className="w-full py-4 bg-gradient-to-r from-[#00f2ff] to-[#7000ff] text-[#0a0a0f] font-bold rounded-xl shadow-xl active:scale-95 tracking-widest uppercase">Start Chat</button>
                </div>
              ) : ( <div className="flex flex-col items-center py-10"><Loader2 className="animate-spin text-[#00f2ff] mb-4" size={32} /><span className="text-[10px] text-[#4a4a6a] tracking-widest uppercase">Loading Vortex Playgrounds</span></div> )}
            </div>
          </div>
        </div>
      )}
      ...
      {isDeleteModalOpen && (
        <div className="fixed inset-0 bg-[#0a0a0f]/80 backdrop-blur-md flex items-center justify-center z-[120] p-4">
          <div className="bg-[#12121e] rounded-3xl w-full max-w-sm p-8 border border-[#1f1f2e] shadow-2xl">
            ...
            <p className="text-xs text-[#8e8eaf] mb-6">This action is irreversible. Type <span className="text-red-500 font-bold">YES</span> to confirm deletion of <span className="text-white font-mono">{sessionToAction?.name || sessionToAction?.model}</span>.</p>
