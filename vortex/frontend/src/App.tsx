...
type Session = { id?: string; name?: string; model: string; created_at: string; is_mock: boolean };
...
function ChatPlatform() {
  ...
  const handleInitialSetup = async () => {
    ...
    try {
      ...
      alert('Verification Failed. Ensure the Python Core is running.');
    } finally {
  ...
  const handleModelChange = async (newModel: string) => {
    ...
    try {
      ...
      alert('Failed to update playground.');
    }
  };
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      alert('Vortex connection failed. Ensure Python Core is reachable.');
    } finally {
      setIsSending(false);
    }
  };
    ...
    try {
      ...
      mediaRecorder.onstop = async () => {
        ...
        try {
          ...
          if (result && result.text) setInputText(prev => (prev ? `${prev} ${result.text}` : result.text));
      ...
      <aside className="w-72 bg-[#0d0d14] border-r border-[#1f1f2e] flex flex-col hidden lg:flex">
        ...
        <div className="p-6 border-t border-[#1f1f2e] bg-[#0a0a0f]/50 backdrop-blur-md">
          <div className="flex flex-col gap-2">
            <div className="flex items-center gap-3 text-xs text-[#4a4a6a]"><div className="w-2 h-2 rounded-full bg-[#00f2ff] animate-pulse"></div><span>ORCHESTRATOR CORE</span></div>
          </div>
        </div>
      </aside>
          ...
          <div className="flex items-center gap-4">
            {activeSession ? (
              <div className="flex flex-col">
                <span className="text-sm font-bold text-[#00f2ff] tracking-wide uppercase">{activeSession.name || `INTELLIGENCE PLAYGROUND`}</span>
                <select value={activeSession.model} onChange={(e) => handleModelChange(e.target.value)} className="text-[11px] text-[#4a4a6a] font-mono bg-transparent border-none outline-none cursor-pointer hover:text-[#00f2ff] transition-colors">
                  {availableModels.map(m => <option key={m} value={m} className="bg-[#12121e]">{m.toUpperCase()}</option>)}
                </select>
              </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 lg:p-10 space-y-8 custom-scrollbar">
          {!activeSession && (
            <div className="h-full flex flex-col items-center justify-center text-center max-w-2xl mx-auto">
              ...
              <h2 className="text-3xl font-bold text-white mb-4 tracking-tight">Vortex Intelligent Terminal</h2>
              <p className="text-[#8e8eaf] text-lg leading-relaxed mb-8">{isSetupComplete ? 'Launch a session to begin unified agent computing.' : 'Please configure your API Key to establish a secure link.'}</p>
              <button onClick={() => protectedAction(handleOpenNewChat)} className="bg-[#00f2ff] hover:bg-[#00d8e6] text-[#0a0a0f] px-8 py-3 rounded-xl font-bold transition-all">{isSetupComplete ? 'Launch Session' : 'Configure Endpoint'}</button>
            </div>
          )}
          {messages.map((msg, i) => (
            <div key={i} className={`flex gap-6 max-w-5xl mx-auto animate-in fade-in slide-in-from-bottom-2 duration-300 ${msg.role === 'user' ? 'justify-end' : ''}`}>
              {msg.role !== 'user' && ( <div className="w-10 h-10 rounded-xl bg-[#1a1a2e] border border-[#00f2ff]/30 flex items-center justify-center flex-shrink-0 shadow-lg"><Bot size={22} className="text-[#00f2ff]" /></div> )}
              <div className={`px-6 py-4 rounded-2xl max-w-[85%] text-[15px] leading-relaxed border ${msg.role === 'user' ? 'bg-gradient-to-br from-[#00f2ff] to-[#7000ff] text-[#0a0a0f] font-medium border-transparent rounded-tr-none shadow-xl' : 'bg-[#12121e] text-[#e2e2e2] border-[#1f1f2e] rounded-tl-none shadow-xl'}`}>
          ...
          {(isSending || isTranscribing) && ( <div className="flex gap-6 max-w-5xl mx-auto"><div className="w-10 h-10 rounded-xl bg-[#1a1a2e] border border-[#00f2ff]/30 flex items-center justify-center flex-shrink-0"><Bot size={22} className="text-[#00f2ff]" /></div><div className="px-6 py-4 rounded-2xl bg-[#12121e] text-[#4a4a6a] border border-[#1f1f2e] flex items-center gap-3"><div className="flex gap-1"><div className="w-1.5 h-1.5 bg-[#00f2ff] rounded-full animate-bounce"></div><div className="w-1.5 h-1.5 bg-[#00f2ff] rounded-full animate-bounce [animation-delay:0.2s]"></div><div className="w-1.5 h-1.5 bg-[#00f2ff] rounded-full animate-bounce [animation-delay:0.4s]"></div></div><span className="text-xs font-mono uppercase tracking-widest text-[#00f2ff]/40">Computing</span></div></div> )}
        ...
        {activeSession && (
          <div className="p-8 bg-gradient-to-t from-[#0a0a0f] via-[#0a0a0f] to-transparent">
            <div className="max-w-4xl mx-auto">
              {stagedFile && (
                <div className="mb-3 flex items-center gap-3 bg-[#12121e] p-2 pr-4 rounded-xl border border-[#00f2ff]/30 animate-in slide-in-from-bottom-2 duration-200">
                  <div className="w-10 h-10 rounded-lg bg-[#1a1a2e] flex items-center justify-center text-[#00f2ff]">
                    {stagedFile.type.startsWith('image/') ? <ImageIcon size={20} /> : <FileText size={20} />}
                  </div>
              ...
              <div className="relative flex items-end gap-3 bg-[#12121e] p-3 rounded-2xl border border-[#1f1f2e] focus-within:border-[#00f2ff]/50 transition-all">
                ...
                <button onClick={handleSendMessage} disabled={!inputText.trim() || isSending || isRecording} className={`p-3 rounded-xl transition-all shadow-lg active:scale-95 ${(!inputText.trim() || isSending || isRecording) ? 'bg-[#1a1a2e] text-[#4a4a6a] cursor-not-allowed opacity-50' : 'bg-gradient-to-br from-[#00f2ff] to-[#7000ff] text-[#0a0a0f]'}`}><Send size={20} /></button>
              </div>
            </div>
          </div>
        )}
      ...
      {isInitialSetupOpen && (
        <div className="fixed inset-0 bg-[#0a0a0f]/95 backdrop-blur-xl flex items-center justify-center z-[100] p-4">
          <div className="bg-[#12121e] rounded-3xl w-full max-w-md p-10 border border-[#1f1f2e] text-center shadow-[0_0_50px_rgba(0,242,255,0.1)]">
            ...
            <h2 className="text-2xl font-bold text-white mb-2">Intelligence Authorization</h2>
            <p className="text-[#4a4a6a] text-sm mb-8">Establish a secure link with the Python Orchestrator.</p>
            <input type="password" value={newApiKey} onChange={e => setNewApiKey(e.target.value)} className="w-full px-5 py-4 bg-[#0a0a0f] border border-[#1f1f2e] rounded-2xl focus:border-[#00f2ff] outline-none transition-all placeholder-[#4a4a6a] text-sm mb-8" placeholder="sk-..." />
            <button onClick={handleInitialSetup} disabled={isUpdatingKey || (!newApiKey.startsWith('sk-') && !MOCK_CONFIG.enabled)} className="w-full py-4 bg-[#00f2ff] hover:bg-[#00d8e6] text-[#0a0a0f] font-bold rounded-2xl transition-all shadow-xl flex items-center justify-center gap-2 tracking-widest uppercase text-xs">{isUpdatingKey ? <Loader2 className="animate-spin" /> : 'Authorize Link'}</button>
          </div>
        </div>
      )}
      ...
      {isSettingsModalOpen && (
        <div className="fixed inset-0 bg-[#0a0a0f]/90 backdrop-blur-md flex items-center justify-center z-[110] p-4">
          <div className="bg-[#12121e] rounded-3xl w-full max-w-md p-8 border border-[#1f1f2e] shadow-2xl relative">
            ...
            <h3 className="text-xl font-bold text-white mb-8 flex items-center gap-3"><Settings className="text-[#00f2ff]" /> Settings</h3>
            <div className="space-y-8">
              <section className="p-6 bg-[#0a0a0f]/50 border border-[#1f1f2e] rounded-2xl">
                ...
                <Link to="/admin/upload-documents" onClick={() => setIsSettingsModalOpen(false)} className="px-4 py-2 bg-[#1a1a2e] text-[#00f2ff] rounded-lg text-[10px] font-bold border border-[#00f2ff]/20 uppercase tracking-widest flex items-center gap-2 hover:bg-[#252545] transition-all"><Database size={12} /> Admin Ingestion</Link>
              </section>
              <section className="p-6 bg-[#0a0a0f]/50 border border-[#1f1f2e] rounded-2xl">
      ...
      {isNewChatModalOpen && (
        <div className="fixed inset-0 bg-[#0a0a0f]/80 backdrop-blur-md flex items-center justify-center z-[110] p-4">
          <div className="bg-[#12121e] rounded-3xl shadow-2xl w-full max-w-md p-8 border border-[#1f1f2e] animate-in zoom-in-95 duration-300 relative overflow-hidden">
            ...
            <div className="flex justify-between items-center mb-8 relative"><h3 className="text-2xl font-bold text-white tracking-tight tracking-widest uppercase text-sm">Initialize Session</h3><button onClick={() => setIsNewChatModalOpen(false)} className="text-[#4a4a6a] hover:text-white"><X /></button></div>
            <div className="space-y-6 relative">
              {availableModels.length > 0 ? (
                <div className="space-y-6 animate-in fade-in slide-in-from-top-2">
                  ...
                  <button onClick={handleStartInteraction} className="w-full py-4 bg-gradient-to-r from-[#00f2ff] to-[#7000ff] text-[#0a0a0f] font-bold rounded-xl shadow-xl active:scale-95 tracking-widest uppercase">Launch Playground</button>
                </div>
              ) : ( <div className="flex flex-col items-center py-10"><Loader2 className="animate-spin text-[#00f2ff] mb-4" size={32} /><span className="text-[10px] text-[#4a4a6a] tracking-widest uppercase">Fetching Vortex Playgrounds</span></div> )}
            </div>
          </div>
        </div>
      )}
